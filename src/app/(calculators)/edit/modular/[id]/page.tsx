...
// Dynamic imports
const EditInputs = dynamic(() => import('./_components/EditInputs'), {
  ssr: false,
});
...
const Edit: NextPage<ModularPageTypes> = async ({ params }) => {
  // Get calculator data
  const calculatorData: Promise<CalculatorTypes> = getModularCalculator({
    calcId: params.id,
  });
    ...
    <div>
      ...
      <div className='custom-bg-color p-3 rounded-lg lg:rounded-box mt-3'>
        <Suspense fallback={<Loader />}>
          <EditInputs
            baseFormula={calculator.formula}
            baseFormulaVariables={calculator.formulaVariables}
            baseTitle={calculator.title}
            baseCategory={calculator.category}
            baseDescription={calculator.description}
            baseInfo={calculator.info}
            baseIsInfoMarkdown={calculator.isInfoMarkdown}
            baseLabels={calculator.inputLabels}
            baseSelects={calculator.inputSelects}
            authorId={calculator.author?._id || ''}
          />
        </Suspense>
      </div>

...
const UnverifiedCalculatorsDashboard: FunctionComponent = () => {
  ...
  const { calculatorsData, error, isLoading, mutate } = useUnverifiedCalcs({
    page,
    sortType,
    search,
    tag,
  ...
  if (calculatorsData && calculatorsData.count === 0 && !isLoading) {
    return (
      <div className=''>
        <div className='text-center mx-auto mt-3 min-h-[150px] max-h-[150px] justify-center'>
          <p>There are no unverified calculators at the moment.</p>
        </div>
      </div>
    );
  }
...
export default UnverifiedCalculatorsDashboard;

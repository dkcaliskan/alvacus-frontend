...
const UnseenReports: FunctionComponent = () => {
  const params = useSearchParams();

  const search = params.get('search') || '';
  const page = params.get('page');

  // Get the unseen contacts data
  const { contactsData, isLoading, error, mutate } = useUnseenContacts({
    page: page || '1',
    sortType: 'recent',
    search: search || '',
  });
